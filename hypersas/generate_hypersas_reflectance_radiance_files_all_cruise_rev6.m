% generate_hypersas_reflectance_radiance_files_all_cruise_rev6.m - Program 
%   to read hypersas data from compiled sensor '*.mat' files, process and plot
% 
% Syntax:  generate_hypersas_reflectance_radiance_files_all_cruise_rev6.m
%
% Inputs:
%    1) Folder location with daily compiled sensor data files ('*.mat')
%      produced by 'compile_hypersas_data_revised_v2.m'
%    2) 'rhoTable_AO2015.txt' file for Mobley (2015) reflectance correction
%    3) met_file_name file (e.g., 'buoy42040_2009_continuous_winds.txt') with NDBC buoy wind data for cruise period
%    4) Spreadsheet with sea condition information ('apr2009_hypersas_file_list.xls')
%
% Outputs:
%    output - Mat file for each cruise day with structured variable 'rad_rrs_output'
%    containing various fields with processed data 
%   
...
%Set up figure window to display results
fig=figure(1);
...
%Lookup table for Mobley (2015)reflectance correction
...
%Currently using Ruddick et al. (2006) correction
...
for cruisen=2 %1:5
    ...
    for file_no=1:filen     
        ...
        %Determine min and max run numbers for good data sets
        ...
        for lrun=min_run:max_run
            ...
            %% This section gathers the irradiance data
            %
            ...
            %% Gather GPS coordinates and times
            ...
            %% ******Correct radiance for surface reflectance - three possible approaches***** 
            ...
            %Lange et al. (2020), Radiometric approach for the detection of picophytoplankton assemblages across oceanic fronts, Optics Express, 28(18), 25682-25705.
            %Ruddick et al. (2006), Seaborne measurements of near infrared water‐leaving reflectance: The similarity spectrum for turbid waters. Limnology and Oceanography, 51(2), pp.1167-1179.
            %Using Ruddick et al. - others provided for information only
            ...
            for ig=1:length(gps_index)
               ...
               if file_no==9 %file_no=9 corresponds to day 118 (April 2009)
            ...
            if discrete_process==1
                %Load discrete sensor data to determine valid time intervals within 60 sample bins
                   ...
                   %Correct discrete radiance data
                    ...
                    %Compute reflected sky discrete radiance
            ...
            %Eliminate lt data with negative slope exceeding threshold in 770-800 nm range
            ...
            %Interpolate irradiance and irradiance to radiance lt_timer 
            ...
            %% Assemble results in output variable 'rad_rrs_output'
            ...
            if ~exist('rad_rrs_output','var')
                ...
                rad_rrs_output=struct('run_no',lrun,'es_time',{{es_time(min_index:max_index)}},...
                    ...
                    'lambda_dlt',{lambda_dlt},'dlt_data_ref_corr',{dlt_dat_corr(dlt_good_index,:)},...
                    'dlt_data',{dlt_dat(dlt_good_index,:)},'alt_dlt_data',{dlt_dat_alt_corr(dlt_good_index,:)},...
                    'dlt_samp_delay',{dlt_samp_delay(dlt_good_index)},'drrs',{drrs},'alt_drrs',{alt_drrs},...
                    'dli_data',{dli_dat_interp(dlt_good_index,:)},...
                    ...
                    'des_time',{{des_time(des_good_index)}},'des_timer',{des_timer(des_good_index)},...
                    'des_data',{des_dat(des_good_index,:)},'des_samp_delay',{des_samp_delay(des_good_index)},...
                    'des_datenum_interp',{des_datenum_interp},'des_data_interp',{des_dat_interp},...
                    ...
                ...
                rad_rrs_output.dlt_data=[rad_rrs_output.dlt_data;{dlt_dat(dlt_good_index,:)}];
                rad_rrs_output.dlt_data_ref_corr=[rad_rrs_output.dlt_data_ref_corr;{dlt_dat_corr(dlt_good_index,:)}];
                rad_rrs_output.alt_dlt_data=[rad_rrs_output.alt_dlt_data;{dlt_dat_alt_corr(dlt_good_index,:)}];
                rad_rrs_output.dli_data=[rad_rrs_output.dli_data;{dli_dat_interp(dlt_good_index,:)}];
            ...
            %% Print graphs if desired (COMMENTED OUT)
