% Absorption-based daily PP calculation
% pp_daily_algorithm_vertical_profile.m - Program to estimate primary production profile from
% from HyperPro-derived ed and kd values and CTD fluorescence estimated aph values and
% extrapolation of light level over photoperiod
%
% Syntax:  pp_daily_algorithm_vertical_profile
%
% Inputs:
...
%       generated using 'ed_calculation_with_unit_conversion_single_station.m'
%   4) 'Stn#_aph_dat*m.mat' files for filterpad aph slope generated using
%       'aph_slope_aph_mean_calculations.m'
%   5) Folder location with compiled SBE911 .cnv data files ('ctd_alldat.mat') produced 
...
%       'aph_slope_aph_mean_calculations.m'
%    3) Compiled SBE911 .cnv data files ('ctd_alldat.mat') produced using 'readctd_nonseabass.m
%
...
for icr = 2:5
    ...
    for istn=1:length(sta_txt)
        ...
        % if strcmp('G3',sta_txt{istn})  % No HyperPro data at G3
        %     continue
        % end
        ...
        % kd490 = pe_dat.Kd490(sta_indx);  % Commented out - no longer needed; kd490 will be retreived from
        ...
        cos_solzen = cos_solzen_wtr./cos_solzen_pro_tm;
        ...
        % Estimate vertical profile of aph440 from CTD fluorescence
        aph440_fluor = [];
        chl_fluor = [];
        for ictd = 1:length(ctd_all)
            depth_indx = find(contains(ctd_all(ictd).names,'Depth'));
            fluor_indx = find(contains(ctd_all(ictd).names,'Fluorescence'));
            % Extract matching CTD data for the current pad station
            if strcmpi(ctd_all(ictd).sta, strrep(sta_txt{istn},'1b','1'))
                [ctd_depth_unique,ia,ic] = unique(ctd_all(ictd).ctddat(:,depth_indx));
                ctd_fluor_unique = ctd_all(ictd).ctddat(ia,fluor_indx);
                ctd_fluor_interp = interp1(ctd_depth_unique,ctd_fluor_unique,profile_depth,'nearest','extrap'); % Find index of matching depths to PUR
                ctd_fluor_interp(ctd_fluor_interp<0) = NaN;
                switch cruisename
                    % Compute aph440 from fluorescence
                    case 'GC1'
                        aph440_fluor = 0.117.*ctd_fluor_interp.^0.981;
                        chl_fluor = 0.367.*ctd_fluor_interp.^0.866;
                    case 'GC2'
                        aph440_fluor = 0.0344.*ctd_fluor_interp.^0.908;
                        chl_fluor = 0.651.*ctd_fluor_interp.^1.23;
                    case 'GC3'
                        aph440_fluor = 0.0326.*ctd_fluor_interp.^0.990;
                        chl_fluor = 0.692.*ctd_fluor_interp.^1.14;
                    case 'GC4'
                        aph440_fluor = 0.117.*ctd_fluor_interp.^0.981;
                        chl_fluor = 0.367.*ctd_fluor_interp.^0.866;
                    case 'GC5'
        ...
        % Select micro vs pico/nano PUR based on aph slope
        ...
        % For sensitivity analysis, adjust parameters individually by +/-50% changes from original WRM values
        ...
        clear P_z_day P_z_std_day P_z_est_day P_z_PAR_day P_z P_z_std P_z_PAR P_z_est
        ...
        for icos = 1:length(cos_indx)
            ...
            P_z = aph440_fluor'.*pmax_aph440_spec.*(1 - exp(-phi.*(PUR_z(:,icos))./(pmax_aph440_spec))); % mol C m-3 h-1
            ...
            P_z_std = chl_fluor'.*pbmax.*(1 - exp(-alphab.*(PAR_z(:,icos))./pbmax))./12000; % mol C m-3 h-1
            
            % PAR-based PP using conventional P-E parameters and constant KPAR
            P_z_PAR = chl_fluor'.*pbmax.*(1 - exp(-alphab.*(PAR_z_constK(:,icos))./pbmax))./12000; % mol C m-3 h-1
            
            % Full spectral PP estimated using predicted pmax_aph440_spec
            P_z_est = aph440_fluor'.*pmax_aph440_spec_est.*(1 - exp(-phi_est.*(PUR_z(:,icos))./(pmax_aph440_spec_est))); % mol C m-3 h-1
        ...
        P_int(istn) = sum(P_z_day.*z_incr,'omitmissing');
        P_est_int(istn) = sum(P_z_est_day.*z_incr,'omitmissing');
        P_std_int(istn) = sum(P_z_std_day.*z_incr,'omitmissing');
        P_constk_int(istn) = sum(P_z_PAR_day.*z_incr,'omitmissing');  % Constant KPAR assumed
        ...
        ax.FontSmoothing="on";
        % ax.FontSize=18;
        % ax.Color=[0.9 0.9 0.9];  % to get a light background color in between 0-1
        ax.XColor='k';
        ax.YColor='k';
        % ax.XTickLabel = {}; % comment this out if don't want X-axis tick_label
        % set(gcf,'Color','W','InvertHardcopy', 'off'); % if want an image background color to save without axis color
...
%% Save output
        ...
        if save_output
            save([folderpath,'P-E\',cruisename,'_',sta_txt{istn},'_PP_profile_dat.mat'],'PP_dat');

            disp(['Printing ','pp_profile_',sta_txt{istn},'.tif']);
            print([folderpath,'P-E\',cruisename,'_pp_profile_',sta_txt{istn},'.tif'],'-dtiff','-r600');
    ...
    all_IPP_tab_profile = [all_IPP_tab_profile;IPP_tab];

end

all_IPP_tab_profile = sortrows(all_IPP_tab_profile,["Cruise","Station"]);

% Save output
% 
if save_output
    save([folderpath,'P-E\all_IPP_table_profile.mat'],'all_IPP_tab_profile');
    
    filename = 'all_IPP_tab_profile.xlsx';
    writetable(all_IPP_tab_profile,[folderpath,'P-E\',filename],'Sheet',1,'Range','A1','WriteMode','overwritesheet')
end
...
hpp = plot(all_IPP_tab_profile.WRM,all_IPP_tab_profile.WIM,'ob','Linewidth',1.5);
hold on
% h1to1 = plot([0,0.15*12],[0,0.15*12],':k','LineWidth',1.5);
h1to1 = plot([0.03,2],[0.03,2],':k','LineWidth',1.5);

[R_IPP,P] = corrcoef(all_IPP_tab_profile.WRM,all_IPP_tab_profile.WIM);
rtmeansq = rmse(all_IPP_tab_profile.WRM,all_IPP_tab_profile.WIM);
rsquared = R_IPP(1,2).^2;

hpe = plot(all_IPP_tab_profile.WRM,all_IPP_tab_profile.WRME,'+m','Linewidth',1.5);
...
legend([hpp,hpe,h1to1],{'WRM vs WIM Profile','WRM vs WRME Profile','1:1 Line'},'Location','northwest');
% 
disp('Completed');
